...
#         wb_file = os.path.join(current_app.config['UPLOADED_DOCUMENTS_DEST'], filename)
#         workbook = xlrd.open_workbook(wb_file)
#         sheet = workbook.sheet_by_index(0)
#         first_row = current_app.config['FIRST_ROW']
#         for rowno in range(first_row, sheet.nrows - current_app.config['SUBSTR_ROW']):
#             drug_exp_date = xlrd.xldate.xldate_as_datetime(sheet.cell(rowno, current_app.config['COL_DATE']).value,
#                                                            workbook.datemode)
#             expdate_row = ExpdateTable()
#             expdate_row.drug_name = sheet.cell(rowno, current_app.config['COL_DRUG_NAME']).value
#             expdate_row.exp_date = drug_exp_date
#             expdate_row.amount = sheet.cell(rowno, current_app.config['COL_AMOUNT']).value
